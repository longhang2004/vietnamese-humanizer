...
Catalog có 43 pattern, mỗi pattern nêu finding type, scope, aggregation, ngoại lệ và rủi ro false positive. Có 103 example với output mode, `context`, `must_preserve`, `must_not_add` và nguồn gốc đánh giá. Có 33 benchmark case ghi expected output mode, context, blocker cụ thể và ràng buộc bảo toàn. JSON Schema kiểm tra kết quả đánh giá thủ công; một case có thể có nhiều reviewer.
