...
def validate_metadata(metadata: dict[str, Any], directory_name: str) -> list[str]:
    errors: list[str] = []
...
def validate_skill(skill_dir: Any) -> list[str]:
    errors: list[str] = []
    ...
    errors.extend(f"{skill_file}: {item}" for item in validate_metadata(metadata, skill_dir.name))
    headings = {
    ...
    for target in LINK_RE.findall(body):
        ...
        reference = skill_dir / clean_target
        if not (reference.is_file() or reference.is_dir()):
            errors.append(f"{skill_file}: link hỏng: {target}")
    return errors


def validate_all(skills_dir: Any) -> list[str]:
